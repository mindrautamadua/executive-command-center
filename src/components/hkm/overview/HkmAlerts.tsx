...
import { hkmAlerts } from "@/lib/hkm-data";
    ...
    <div className="card anim-rise px-4 pb-3.5 pt-3" style={{ "--d": "200ms" } as React.CSSProperties}>
      ...
      <div className="mt-2.5 grid grid-cols-4 gap-2.5">
        {hkmAlerts.map((a) => {
          ...
          return a.href ? (
            <Link key={a.title} href={a.href} className={`${cls} transition-shadow hover:shadow-card`}>
              {body}
            </Link>
          ) : (
            <div key={a.title} className={cls}>
              {body}
            </div>
          );
        })}
