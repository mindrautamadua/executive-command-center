...
import { keamananCrossLinks, keamananDecisions } from "@/lib/hse-data-detail";
...
/** Keputusan BOD terkait keamanan aset + rute pendalaman lintas dimensi. */
export function KeamananDecisionCenter() {
  return (
    <div
      className="card anim-rise px-4 pb-3.5 pt-3"
      style={{ "--d": "120ms" } as React.CSSProperties}
    >
      <SectionHead title="Decision Center — Keamanan Aset" action="Lihat Semua" badge={<ScopeNote />} />

      <div className="mt-2.5 grid grid-cols-2 gap-3">
        {keamananDecisions.map((d) => {
          const t = TONE[d.tone];
          const Icon = t.icon;
          return (
