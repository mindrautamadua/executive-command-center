...
import { AlertCircle, AlertTriangle, CheckCircle2, ExternalLink, Sparkles } from "lucide-react";
import { siberDecisions, siberRiskLink } from "@/lib/tik-data";
import { SectionHead } from "@/components/hc/SectionHead";
import { ScopeNote } from "@/components/ui/ScopeNote";

const TABS = [
  { label: "Perlu Keputusan", count: siberDecisions.length },
...
export function SiberDecisionCenter() {
  const [tab, setTab] = useState(0);

  return (
    <div className="card anim-rise px-4 pb-3.5 pt-3" style={{ "--d": "270ms" } as React.CSSProperties}>
      <SectionHead title="BOD Decision Center — Keamanan Siber" action="Lihat Semua" badge={<ScopeNote />} />
      ...
      <Link
        href={siberRiskLink.href}
        className="mt-2.5 flex items-start gap-2 rounded-xl border border-[#d3e3f6] bg-[#f1f7fd] px-3 py-2.5 transition-colors hover:border-[#2f6fe4]"
      >
        <ExternalLink size={13} className="mt-[1px] shrink-0 text-[#2f6fe4]" />
        <p className="text-[9px] leading-[1.45] text-ink-500">
          <span className="font-bold text-[#2f6fe4]">{siberRiskLink.label} → </span>
          {siberRiskLink.catatan}
        </p>
      </Link>
    </div>
  );
