...
import { allRisksKpi } from "@/lib/prr-registry";
      ...
      {allRisksKpi.map((k, i) => (
        ...
        >
          <div className="text-[9.5px] font-semibold text-ink-500">
            <span className={k.label === "Total Risks" ? "" : LABEL_TONE[k.tone]}>{k.label}</span>
          </div>
          <div className="mt-2 flex items-end justify-between gap-2">
            <span className="whitespace-nowrap text-[24px] font-extrabold leading-none tracking-[-0.02em] text-ink-900">
              {k.value}
              {k.suffix && <span className="ml-1 text-[11px] font-bold text-ink-400">{k.suffix}</span>}
            </span>
            {k.share && (
              <span className={`text-[10px] font-bold ${SHARE_TONE[k.tone]}`}>{k.share}</span>
            )}
          </div>
