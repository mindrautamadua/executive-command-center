import { ArrowRight, CircleCheck, Leaf, ShieldAlert, TriangleAlert } from "lucide-react";
import { diInsight } from "@/lib/di-data";
import { InklusiIlustrasi } from "./InklusiIlustrasi";

/** Tone chip dark-safe dari globals.css — bukan pastel inline. */
const TONE = {
  success: { chip: "tone-green", Icon: Leaf },
  danger: { chip: "tone-red", Icon: ShieldAlert },
  warning: { chip: "tone-amber", Icon: TriangleAlert },
  info: { chip: "tone-teal", Icon: CircleCheck },
} as const;

export function InsightDi() {
  return (
    <div
    ...
    >
      ...
      <div className="relative z-10 mt-2 flex min-h-0 flex-1 flex-col justify-around pr-[176px]">
        {diInsight.map((it) => {
      ...
      {/* ilustrasi inklusi */}
      <div className="pointer-events-none absolute bottom-2 right-3 w-[186px]">
        <p className="text-center text-[11.5px] font-extrabold leading-[1.3] text-[#1b3a6b]">
          Beragam, Inklusif,
          <br />
          Setara untuk Semua
        </p>
        <InklusiIlustrasi className="mt-1 h-[104px] w-full" />
      </div>
    </div>
  );
}
