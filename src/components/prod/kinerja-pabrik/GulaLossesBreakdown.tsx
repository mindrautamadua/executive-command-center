...
import { gulaLosses, gulaLossesNote } from "@/lib/pabrik-data";
import { useSubholding } from "@/components/SubholdingProvider";
import { inScope, ScopeEmpty } from "@/components/ui/CommodityScope";
import { SectionHead } from "../../hc/SectionHead";

const MAX_PCT = 1.6; // skala bar horizontal

const num = (v: number) =>
  v.toLocaleString("id-ID", { minimumFractionDigits: 2, maximumFractionDigits: 2 });

export function GulaLossesBreakdown() {
  const { active, def } = useSubholding();
  // Seluruh kartu ini milik SugarCo: kehilangan gula diukur terhadap pol tebu.
  const dalamCakupan = inScope(active, "gula tebu giling");
                  ...
                  <div className="relative mt-[4px] h-[7px] overflow-hidden rounded-full bg-[#eef2f6]">
                    <span
                      className={`block h-full rounded-full ${
                        overNorma ? "bg-[#ef4444]" : "bg-ptpn-green"
                      }`}
