...
import { pgdIntelligence } from "@/lib/pgd-data";
...
const counts = (["red", "amber", "green"] as const).map((tone) => ({
  tone,
  label: tone === "red" ? "Kritis" : tone === "amber" ? "Perhatian" : "Positif",
  value: pgdIntelligence.filter((s) => s.tone === tone).length,
}));

/** Sintesis eksekutif pengadaan: sinyal kritis + satu rekomendasi utama. */
export function PgdIntelligence() {
  return (
    <div className="card anim-rise px-4 pb-3.5 pt-3" style={{ "--d": "30ms" } as React.CSSProperties}>
      ...
      <div className="mt-2.5 flex items-start gap-2 rounded-xl bg-ptpn-greenLight px-3 py-2.5">
        ...
        <p className="text-[9.5px] leading-[1.45] text-ink-900">
          ...
          Pangkas siklus pengadaan ke 38 hari lewat SLA evaluasi 7 hari agar realisasi capex 32,1%
          terkejar, sambil mengunci kontrak payung pupuk H2 2026 Rp 2,1 T untuk meredam eksposur
          harga +Rp 242 M dan menurunkan konsentrasi Top-20 dari 43%.
        </p>
      </div>
    </div>
  );
