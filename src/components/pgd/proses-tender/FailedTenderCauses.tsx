...
import { failedTenderCauses } from "@/lib/pgd-data-detail";
import { CHART_AXIS, CHART_TOOLTIP_STYLE, PALETTE } from "@/lib/chart-palette";
import { SectionHead } from "@/components/hc/SectionHead";
import { ScopeNote } from "@/components/ui/ScopeNote";

const TOTAL = failedTenderCauses.reduce((s, r) => s + r.jumlah, 0);
...
const rows = failedTenderCauses.map((r) => ({
  ...r,
  short: r.penyebab.length > 30 ? `${r.penyebab.slice(0, 29)}…` : r.penyebab,
}));

export function FailedTenderCauses() {
  return (
    <div
      className="card anim-rise flex h-full flex-col px-4 pb-2.5 pt-3"
      style={{ "--d": "60ms" } as React.CSSProperties}
    >
      <SectionHead title="Penyebab Tender Gagal / Ulang" badge={<ScopeNote />} />
      <p className="mt-[3px] text-[9px] text-ink-500">
        {TOTAL} Tender Gagal · <span className="font-bold text-[#8b5cf6]">11,2%</span> dari 268
        Tender Selesai YTD
      </p>
      ...
      <div className="mt-1 min-h-0 w-full flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart
            data={rows}
            layout="vertical"
            margin={{ top: 4, right: 28, bottom: 0, left: 6 }}
            barCategoryGap="22%"
          >
