import { ArrowRight, Eye, ScanSearch } from "lucide-react";
import type { DekInsight } from "@/lib/dek-data";

/**
 * Kartu Insight & Tindak Pengawasan dimensi Dewan Komisaris (mirror
 * TikInsightGrid) — tiap item memasangkan temuan pengawasan dengan langkah
 * pengawasan lanjutannya, bukan keputusan eksekusi.
 */
export function DekInsightGrid({ items, cols }: { items: DekInsight[]; cols: string }) {
  return (
    <div
        ...
        {items.map((ins) => (
          <div key={ins.insight} className="flex items-start gap-2.5">
            <span className="flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-full bg-[#e8f1fd] text-[#2f6fe4]">
              <ScanSearch size={14} strokeWidth={1.9} />
            </span>
            <div className="min-w-0">
              ...
              <p className="mt-[4px] flex items-start gap-1 text-[8.5px] leading-snug text-ink-500">
                <Eye size={9} className="mt-[1px] shrink-0 text-ptpn-green" />
                <span>
