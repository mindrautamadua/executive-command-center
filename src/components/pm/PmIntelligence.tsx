...
import { pmIntelCounts, pmIntelRecommendation, pmIntelSignals } from "@/lib/pm-data";
...
/** Sintesis eksekutif People Math & HPI: sinyal utama + satu rekomendasi prioritas. */
export function PmIntelligence() {
  return (
    <div className="card anim-rise px-4 pb-3.5 pt-3" style={{ "--d": "30ms" } as React.CSSProperties}>
      ...
      <div className="mt-2.5 flex items-start gap-2 rounded-xl bg-ptpn-greenLight px-3 py-2.5">
        ...
        <p className="text-[9.5px] leading-[1.45] text-ink-900">
          <span className="font-bold text-ptpn-green">Priority Intervention: </span>
          {pmIntelRecommendation}
        </p>
      </div>
    </div>
  );
