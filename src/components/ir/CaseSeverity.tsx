  ...
  return (
    ...
    >
      ...
      <div className="mt-1 flex items-center gap-3">
        {severityDist.map((s) => (
          <span key={s.level} className="flex items-center gap-1 text-[8px] text-ink-500">
      ...
      <div className="mt-2 grid grid-cols-[minmax(0,1fr)_50px_40px_84px] items-center gap-x-2 border-b border-[#eef2f6] pb-1.5 text-[8px] font-semibold uppercase tracking-[0.04em] text-ink-400">
      ...
      <ul className="flex min-h-0 flex-1 flex-col justify-around">
        {severityCases.map((c) => (
          ...
          >
            ...
            <span>
              <span
                className={`inline-block rounded-md px-1.5 py-[3px] text-[8px] font-bold ${SEVERITY_STYLE[c.severity]}`}
              >
                {c.severity}
              </span>
            </span>
